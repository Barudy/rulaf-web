  ...
  return (
    ...
    <div className="max-w-3xl mx-auto bg-white dark:bg-[#171A21] border border-gray-200 dark:border-[#1793D1] rounded-sm p-8 shadow-md dark:shadow-[0_0_15px_rgba(23,147,209,0.3)] transition-all duration-300">
        ...
        <div className="space-y-4 text-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={profil.nama} onChange={(e)=>setProfil({...profil, nama: e.target.value})} className="w-full bg-white dark:bg-[#171A21] border border-gray-300 dark:border-gray-700 rounded px-3 py-2 text-gray-900 dark:text-white text-sm focus:border-[#1793D1] focus:outline-none" placeholder="Nama anda..." />
            </div>
            <div>
              <label className="text-[#1793D1] font-bold block mb-1">UMUR:</label>
              <input type="number" value={profil.umur} onChange={(e)=>setProfil({...profil, umur: e.target.value})} className="w-full bg-white dark:bg-[#171A21] border border-gray-300 dark:border-gray-700 rounded px-3 py-2 text-gray-900 dark:text-white text-sm focus:border-[#1793D1] focus:outline-none" placeholder="Umur..." />
            </div>
            <div>
              <label className="text-[#1793D1] font-bold block mb-1">JANTINA:</label>
              <select value={profil.jantina} onChange={(e)=>setProfil({...profil, jantina: e.target.value})} className="w-full p-2 bg-white dark:bg-[#171A21] border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white outline-none focus:border-[#1793D1]">
        ...
        <div className="mt-10 border-t border-gray-700 pt-6">
          ...
          {profil.peranan === 'Guru' ? (
            <div className="bg-gray-200 dark:bg-black p-4 border border-gray-700 border-l-4 border-l-purple-500 text-sm">
              <p className="text-purple-400 font-bold mb-2">[ AKSES GURU DIBENARKAN ]</p>
              <p className="text-black dark:text-gray-300">Anda mempunyai akses untuk memuat naik modul dan mengubah suai markah Jawi/Al-Quran pelajar di dalam pangkalan data pusat.</p>
              <button className="mt-4 bg-purple-600 text-black dark:text-white px-4 py-2 font-bold hover:bg-purple-500 transition-colors" onClick={()=> router.push('/admin')}>
                Buka Panel Penggredan (Admin)
              </button>
            </div>
          ) : profil.peranan === 'Ibu Bapa' ? (
            <div className="bg-gray-200 dark:bg-black p-4 border border-gray-700 border-l-4 border-l-yellow-500 text-sm">
              <p className="text-yellow-400 font-bold mb-2">[ AKSES IBU BAPA ]</p>
              <p className="text-black dark:text-gray-300">Pantau prestasi anak-anak anda, dan sertai perbincangan komuniti PAK21 untuk mengetahui perkembangan terkini inovasi RuLaF.</p>
              <button className="mt-4 bg-yellow-600 text-black dark:text-white px-4 py-2 font-bold hover:bg-yellow-500 transition-colors" onClick={()=> router.push('/semakan')}>
                Akses Semakan Waris
              </button>
            </div>
          ) : (
            <div className="bg-gray-200 dark:bg-black p-4 border border-gray-700 border-l-4 border-l-green-500 text-sm">
              <p className="text-green-400 font-bold mb-2">[ AKSES MURID ]</p>
              <p className="text-black dark:text-gray-300">Lihat prestasi formatif, pencapaian gamifikasi, dan sejarah semakan markah Jawi terkini anda di sini.</p>
              <button className="mt-4 bg-green-600 text-black dark:text-white px-4 py-2 font-bold hover:bg-green-500 transition-colors" onClick={()=> router.push('/semakan')}>
                Semak Markah Saya
              </button>
            </div>
          )}
